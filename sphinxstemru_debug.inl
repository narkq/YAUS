//
// $Id: sphinxstemru.inl 555 2007-02-09 19:05:39Z shodan $
//
...
#if SPH_STEM_DBG
	printf("entering table, l = '%c'\n", l);
#endif
	for ( i=0, j=0; i<icount; i++ )
	{
#if SPH_STEM_DBG
		printf("  i=%d\tj=%d\tfirst='%c'\n", i, j, itable[i].first);
#endif
		if ( l==itable[i].first )
		{
			m = itable[i].count;
			i = j-1;
#if SPH_STEM_DBG
			printf("    first char matched, m=%d\ti=%d\n", m, i);
#endif
			while ( m-- )
			{
				i++;
				...
				k = len;
#if SPH_STEM_DBG
				printf("      before loop, i=%d\tj=%d\tk=%d\n", i, j, k);
#endif
				if ( j>k )
					continue;
				for ( ; j>=0; k--, j-- )
				{
#if SPH_STEM_DBG
					printf("        in     loop, i=%d\tj=%d\tk=%d\tw='%c'\tt='%c'\n", i, j, k, word[k], table[i].suffix[j]);
#endif
...
void LOC_PREFIX(stem_ru_init) ()
{
	...
	#define STEM_RU_INIT_TABLE(table) \
		...
		{ \
			...
			printf(s, (int)strlen((char*)LOC_PREFIX(table)[i].suffix), sizeof(LOC_CHAR_TYPE), LOC_PREFIX(table)[i].len); \
		}
#else
	#undef STEM_RU_INIT_TABLE
	#define STEM_RU_INIT_TABLE(table) \
